...
Shim.prototype.setInternalProperty = psemver.satisfies('4 - 7')
                                   ? setInternalPropertyNoTryCatch
                                   : setInternalProperty
...
function record(nodule, properties, recordNamer) {
  ...
  return this.wrap(nodule, properties, function makeWrapper(shim, fn, name) {
    ...
    function _applyRecorderSegment(segment, ctx, args, segDesc) {
      ...
      var promised = false
      try {
        var ret = shim.applySegment(fn, segment, true, ctx, args)
        if (segDesc.after && segDesc.promise && shim.isPromise(ret)) {
          promised = true
          return ret.then(function onThen(val) {
            segment.touch()
            segDesc.after(shim, fn, name, null)
            return val
          }, function onCatch(err) {
            segment.touch()
            segDesc.after(shim, fn, name, err)
        ...
        if (segDesc.after && (error || !promised)) {
          segDesc.after(shim, fn, name, error)
        }
      }
    }
  })
...
function bindSegment(nodule, property, segment, full) {
  ...
  // In order to maintain arity on older versions of node we wrap all our
  // wrappers in an eval'd function. We need to store the segment on this outer
  // function.
  // TODO: Remove this once Node v0.12 is deprecated.
  this.storeSegment(property ? wrapped[property] : wrapped, segment)
...
function setInternalProperty(obj, name, val) {
  ...
  var hideInternal = this.agent.config.transaction_tracer.hide_internals

  try {
    if (hideInternal && !hasOwnProperty(obj, name)) {
...
/**
 * On node versions 0.10 and 0.12 calling Object.getOwnPropertyDescriptor on a
 * with a non-object throws an exception. On node versions 8+ using a
 * try/catch is faster.  On node version 4 and 6 omitting the try/catch
 * is faster.
 */
function setInternalPropertyNoTryCatch(obj, name, val) {
  if (!obj || !name) {
    this.logger.debug('Not setting property; object or name is missing.')
    return obj
  }

  var hideInternal = this._agent.config.transaction_tracer.hide_internals
  var propDesc = Object.getOwnPropertyDescriptor(obj, name)
  if (hideInternal && !propDesc && !Object.isFrozen(obj)) {
    Object.defineProperty(obj, name, {
      enumerable: false,
      writable: true,
      value: val
    })
  } else if (propDesc && propDesc.writable && !Object.isSealed(obj)) {
    obj[name] = val
  } else {
    this.logger.debug('Failed to set property "%s" to %j', name, val)
  }
  return obj
}
