...
function Shim(agent, moduleName, resolvedName) {
  ...
  this._toExport = null
  this._debug = false
  if (agent.config.transaction_tracer.enumerable_internals) {
    this.moduleName = moduleName
  } else {
    this.defineProperty(this, 'moduleName', moduleName)
  }

  // Determine the root directory of the
  var moduleRoot = null
...
function setInternalProperty(obj, name, val) {
  ...
  try {
    if (!hasOwnProperty(obj, name)) {
      if (this._agent.config.transaction_tracer.enumerable_internals) {
        obj[name] = val
      } else {
...
function setInternalPropertyNoTryCatch(obj, name, val) {
  ...
  if (!propDesc && !Object.isFrozen(obj)) {
    if (this._agent.config.transaction_tracer.enumerable_internals) {
      obj[name] = val
    } else {
      Object.defineProperty(obj, name, {
